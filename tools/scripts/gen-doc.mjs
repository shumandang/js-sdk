...
if (FLAG === '--open') {
  ...
  livever.default.start({
    port: 4004, // Set the ver port. Defaults to 8080.
    host: '0.0.0.0', // Set the address to bind to. Defaults to 0.0.0.0 or process.env.IP.
    root: './doc', // Set root directory that's being ved. Defaults to cwd.
    open: false, // When false, it won't load your brow by default.
    ignore: 'scss,my/templates', // comma-separated string for paths to ignore
    file: 'index.html', // When set, ve this file (ver root relative) for every 404 (useful for single-page applications)
    wait: 1000, // Waits for all changes, before reloading. Defaults to 0 sec.
    // mount: [['/components', './node_modules']], // Mount a directory to a route.
    logLevel: 2, // 0 = errors only, 1 = some, 2 = lots
    // middleware: [function(req, res, next) { next(); }] // Takes an array of Connect-compatible middleware that are injected into the ver middleware stack
  });
} else if (FLAG === '--push') {
